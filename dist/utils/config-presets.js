...
export function getPresetConfig(chainId) {
    ...
    if (!preset) {
        throw new Error(`No QSSN preset configuration found for chainId ${chainId}. ` +
...
export function createQssnConfig(userConfig) {
    ...
    if (paymasterFieldsProvided > 0 && paymasterFieldsProvided < 3) {
        const missing = [];
        if (!hasPaymasterUrl)
            missing.push("paymasterUrl");
        if (!hasPaymasterAddress)
            missing.push("paymasterAddress");
        if (!hasPaymasterToken)
            missing.push("paymasterToken");
        throw new Error(`Incomplete paymaster configuration. To use a paymaster, you must provide all three fields: ` +
