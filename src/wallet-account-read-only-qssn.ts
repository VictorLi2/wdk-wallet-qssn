...
export class WalletAccountReadOnlyQssn {
  ...
  async getAddress(): Promise<string> {
    ...
    const factory = new Contract(
      this._config.factoryAddress,
      FACTORY_ABI,
      provider
    );

    const mldsaPublicKeyHex =
      "0x" + Buffer.from(this._mldsaPublicKey).toString("hex");
    this._walletAddress = await factory.getWalletAddress(
      mldsaPublicKeyHex,
      this._ecdsaOwner
    );
  ...
  async quoteSendTransaction(
    ...
    try {
      ...
      const adjustedTotalGas =
        adjustedCallGas + adjustedVerificationGas + adjustedPreVerificationGas;
  ...
  protected async _estimateUserOperationGas(
    ...
    const { callGasLimit, verificationGasLimit, preVerificationGas, totalGasEstimate } =
      result.result;
