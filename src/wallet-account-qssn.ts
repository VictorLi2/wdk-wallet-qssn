...
export class WalletAccountQssn extends WalletAccountReadOnlyQssn {
  ...
  constructor(
    ...
    if (
      this._ecdsaWallet.address.toLowerCase() !==
      ownerAccount._address.toLowerCase()
    ) {
      throw new Error(
        `ECDSA wallet address mismatch: ${this._ecdsaWallet.address} !== ${ownerAccount._address}`
      );
    }
  }
  ...
  async sendTransaction(
    ...
    const { fee, gasLimits, _cached } = await this.quoteSendTransaction(
      tx,
      config
    );
  ...
  async toReadOnlyAccount(): Promise<WalletAccountReadOnlyQssn> {
    ...
    const readOnlyAccount = new WalletAccountReadOnlyQssn(
      ecdsaOwner,
      mldsaPublicKey,
      this._config
    );
  ...
  protected override async _getProvider(): Promise<
  ...
  private async _buildUserOp(
    ...
    const basePreVerificationGas =
      gasLimits?.preVerificationGas || BigInt(isDeployed ? 150000 : 500000);
    const estimatedVerificationGas =
      gasLimits?.verificationGasLimit || BigInt(isDeployed ? 196608 : 1000000);
    // Enforce minimum for QSSN wallets (dummy sig estimation underestimates real sig gas)
    const baseVerificationGasLimit =
      estimatedVerificationGas > QSSN_MIN_VERIFICATION_GAS
  ...
  private async _sendUserOperation(
    txs: EvmTransaction[],
    options?: UserOpBuildOptions
  ): Promise<string> {
    ...
    const mldsaSignatureHex = await this._mldsaAccount.sign(
      getBytes(userOpHash)
    );
    const mldsaPublicKeyHex = this._mldsaAccount.publicKeyHex;
    ...
    const bundlerParams: (UserOp | string | object)[] = [
      userOp,
      this._config.entryPointAddress,
    ];
    ...
    if (result.error) {
      ...
      if (result.error.message && result.error.message.includes("AA50")) {
        throw new Error(
          "Not enough funds on the wallet account to repay the paymaster."
        );
      }
