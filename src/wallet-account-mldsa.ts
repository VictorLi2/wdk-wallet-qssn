...
export class WalletAccountMldsa {
  ...
  constructor(
    ...
    const seedBytes =
      typeof seed === "string" ? mnemonicToSeedSync(seed) : seed;
    ...
    if (![44, 65, 87].includes(this._securityLevel)) {
      throw new Error(
        `Invalid security level: ${this._securityLevel}. Must be 44, 65, or 87.`
      );
    }

    // Generate full BIP32 path using the crypto module
    const fullPath = MLDSAKeyDerivation.getDerivationPath(
      path,
      this._securityLevel
    );

    // Derive ML-DSA key pair using the crypto module
    this._keyPair = MLDSAKeyDerivation.deriveKeyPair(
      seedBytes,
      fullPath,
      this._securityLevel
    );
